...
async function snap() {
  return p.evaluate(() => {
    ...
    return { t: performance.now(), v: Number(m._wasm_vclock()), insns: Number(m._wasm_insns()),
      ...
      lcCall: g(55), difMuxRebuild: g(59), difTxWord: g(60), dmacBurst: g(61), dmacSchedTimer: g(62), dmacXlatFill: g(63), gptuTimer: g(64) };
  });
}
...
const rec = {
  ...
  perS: { lcCall: Math.round(d.lcCall / wall), difMuxRebuild: Math.round(d.difMuxRebuild / wall), difTxWord: Math.round(d.difTxWord / wall),
    dmacBurst: Math.round(d.dmacBurst / wall), dmacSchedTimer: Math.round(d.dmacSchedTimer / wall), dmacXlatFill: Math.round(d.dmacXlatFill / wall), gptuTimer: Math.round(d.gptuTimer / wall) },
};
await shoot("end");
