...
[ "$REV" = "$PIN" ] || [ "${FORCE:-0}" = "1" ] ||
  { echo "qemu is at $REV, versions.env pins $QEMU_PMB887X_REV — refusing (FORCE=1 to override)" >&2; exit 1; }
...
if ! grep -q "^diff --git" "$TMP/wip.patch"; then
  echo "capture-patch: no uncommitted changes in qemu beyond patches/ (nothing to do)"
  exit 0
fi
